...
日本語学習者がひらがなを「書いて」覚えるモバイルファーストの学習 Web アプリ（PWA）です。

## できること

- **勉強モード**: 行別の一覧から1文字を選び、書き順アニメーションを見ながら5回書く
- **学習記録**: 五十音表で進捗を確認し、習得済み状態を手動で設定・解除してLocalStorageに保存
- **韓国語UI**: 操作文言を韓国語で表示
...
- **学習ステータス**: 書いた回数が0回なら未学習、1回以上なら学習中。習得済みは利用者が手動で設定・解除する
- **LocalStorage**: `version` フィールド付き。v1の書き記録と旧習得状態をv2へ移行する
...
ひらがなの書き順SVGは [animCJK](https://github.com/parsimonhi/animCJK)
